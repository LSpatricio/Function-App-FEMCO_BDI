...
namespace FUNCTION_FEMCO_BDI.Table.Custom.AUDIT_FEMCO
{
    public class Function_AUDIT_FEMCO
    {
        private readonly ILogger _logger;
        private readonly DAO_SQL _dao;
        private readonly ICMService _icmservice;
        private const string NOMBRE_TABLA = "FEMCO.AUDIT_";

        public Function_AUDIT_FEMCO(ILoggerFactory loggerFactory, DAO_SQL dao, ICMService icmService)
        {
            _logger = loggerFactory.CreateLogger<Function_AUDIT_FEMCO>();
            _dao = dao;
        ...
        public async Task<string> BulkCreate_AUDIT_FEMCO()
        {
        ...
        [Function("BulkCreate_Trigger_AUDIT_FEMCO")]
        public async Task<HttpResponseData> BulkCreate_Trigger_AUDIT_FEMCO([HttpTrigger(AuthorizationLevel.Function, "post", Route = "BulkCreate_Trigger_AUDIT_FEMCO")] HttpRequestData req)
        {
            var response = req.CreateResponse();

            try
            {
                _logger.LogInformation("Inicio de la función BulkCreate_Trigger_AUDIT_FEMCO.");
                response.Headers.Add("Content-Type", "application/json; charset=utf-8");
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
                string mensaje = await BulkCreate_AUDIT_FEMCO();
        ...
        public async Task BulkCreate_Timer_AUDIT_FEMCODailyTask([TimerTrigger("0 30 * 1-4 * *")] TimerInfo myTimer)
        {
            //Expresion cron
            //azure
            // segundos , minutos, horas, dias, mes, dia (Lunes a Domingo)

            _logger.LogInformation("Inicio de la función BulkCreate_Timer_AUDIT_FEMCO");
