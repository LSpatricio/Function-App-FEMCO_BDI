...
namespace FUNCTION_FEMCO_BDI.Table.Custom.ADMIN_FEMCO
{
    public class Function_ADMIN_FEMCO_FEMCO
    {
        private readonly ILogger _logger;
        private readonly DAO_SQL _dao;
        private readonly ICMService _icmservice;
        private const string NOMBRE_TABLA = "FEMCO.ADMIN";

        public Function_ADMIN_FEMCO_FEMCO(ILoggerFactory loggerFactory, DAO_SQL dao, ICMService icmService)
        {
            _logger = loggerFactory.CreateLogger<Function_ADMIN_FEMCO_FEMCO>();
            _dao = dao;
        ...
        public async Task<string> BulkCreate_ADMIN_FEMCO()
        {
        ...
        [Function("BulkCreate_Trigger_ADMIN_FEMCO")]
        public async Task<HttpResponseData> BulkCreate_Trigger_ADMIN_FEMCO([HttpTrigger(AuthorizationLevel.Function, "post", Route = "BulkCreate_Trigger_ADMIN_FEMCO")] HttpRequestData req)
        {
            _logger.LogInformation("Inicio de la función BulkCreate_Trigger_ADMIN_FEMCO.");
            var response = req.CreateResponse();
            response.Headers.Add("Content-Type", "application/json; charset=utf-8");

            try
            {
                string mensaje = await BulkCreate_ADMIN_FEMCO();
            ...
            {
                _logger.LogInformation("Fin de la función BulkCreate_Trigger_ADMIN_FEMCO");
        ...
        //1 al 4 de cada mes al minuto 30 cada hora

        [Function("BulkCreate_Timer_ADMIN_FEMCO")]
        public async Task BulkCreate_Timer_ADMIN_FEMCO([TimerTrigger("0 30 * 1-4 * *")] TimerInfo myTimer)
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Error al ejecutar la función BulkCreate_Timer_ADMIN_FEMCO: {Message}", ex.Message);
