...
namespace FUNCTION_FEMCO_BDI.Table.Custom.CONFIGDATESTRINGWEEKS
{
    public class Function_CONFIGDATESTRINGWEEKS
    {
        private readonly ILogger _logger;
        private readonly DAO_SQL _dao;
        private readonly ICMService _icmservice;
        private const string NOMBRE_TABLA = "FEMCOVS.CONFIGDATESTRINGWEEKS";

        public Function_CONFIGDATESTRINGWEEKS(ILoggerFactory loggerFactory, DAO_SQL dao, ICMService icmService)
        {
            _logger = loggerFactory.CreateLogger<Function_CONFIGDATESTRINGWEEKS>();
            _dao = dao;
        ...
        public async Task<string> BulkCreate_CONFIGDATESTRINGWEEKS()
        {
            string modeloICM = Environment.GetEnvironmentVariable("ModelFemcoVS");
            string TablaICM = "ConfigDateStringWeeks";
        ...
        [Function("BulkCreate_Trigger_CONFIGDATESTRINGWEEKS")]
        public async Task<HttpResponseData> BulkCreate_Trigger_CONFIGDATESTRINGWEEKS([HttpTrigger(AuthorizationLevel.Function, "post", Route = "BulkCreate_Trigger_CONFIGDATESTRINGWEEKS")] HttpRequestData req)
        {
            _logger.LogInformation("Inicio de la función BulkCreate_Trigger_CONFIGDATESTRINGWEEKS.");
            var response = req.CreateResponse();
            response.Headers.Add("Content-Type", "application/json; charset=utf-8");

            try
            {
                string mensaje = await BulkCreate_CONFIGDATESTRINGWEEKS();
            ...
            {
                _logger.LogInformation("Fin de la función BulkCreate_Trigger_CONFIGDATESTRINGWEEKS");
        ...
        //Todos los dias 1:00 am
        [Function("BulkCreate_Timer_CONFIGDATESTRINGWEEKS")]
        public async Task BulkCreate_Timer_CONFIGDATESTRINGWEEKS([TimerTrigger("0 0 1 4-6 * *")] TimerInfo myTimer)
        {
            ...
            {
                _logger.LogError(ex, "Error al ejecutar la función BulkCreate_Timer_CONFIGDATESTRINGWEEKS: {Message}", ex.Message);
            }
            finally
            {
                _logger.LogInformation("Fin de la función BulkCreate_Timer_CONFIGDATESTRINGWEEKS.");
            }
        }
